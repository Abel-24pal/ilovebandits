...
from ..constants import DEFAULT_MAB_SAMPLES_FOR_FREQ_EST
...
class UCBAgent(BaseAgent):
    ...
    def take_action(
        ...
        float - the probability of selecting the action.
        """
        uncertainty = ucb_uncertainty(
            c=self.c, arm_count_updates=self.q_estimator.arm_count_updates
        )
...
class TSAgent(BaseAgent):  # Thompson Sampling Agent for discrete 0-1 rewards
    ...
    def __init__(
        ...
        a_init: float = 1,
        b_init: float = 1,
        samples_for_freq_est: int = DEFAULT_MAB_SAMPLES_FOR_FREQ_EST,
    ):
        ...
        super().__init__(
            q_estimator=QThompSamp(
                alphas=a_init * np.ones(arms), betas=b_init * np.ones(arms)
            )
        )  # it executes the __init__ function of the base class
        self.reset_agent()
    ...
    def _freq_arms(self) -> Dict[int, float]:
        ...
        try:
            ...
            dic_arm_freq.update(
                {
                    arm: None
                    for arm in range(self.q_estimator.arms)
                    if arm not in dic_arm_freq.keys()
                }
            )
